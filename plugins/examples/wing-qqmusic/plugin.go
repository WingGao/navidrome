...
func (WingQQMusicAgent) OnInit(ctx context.Context, req *api.InitRequest) (*api.InitResponse, error) {
	...
	if _baseURL, ok := req.Config["baseURL"]; !ok {
		return &api.InitResponse{Error: "没有baseURL的配置"}, nil
	} else {
		baseURL = _baseURL
	}
...
func (WingQQMusicAgent) GetArtistBiography(ctx context.Context, req *api.ArtistBiographyRequest) (*api.ArtistBiographyResponse, error) {
	var bio string
	var err error
...
func (WingQQMusicAgent) GetAlbumInfo(context.Context, *api.AlbumInfoRequest) (*api.AlbumInfoResponse, error) {
	return nil, ErrNotImplemented
}
